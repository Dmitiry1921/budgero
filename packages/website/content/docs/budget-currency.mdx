...
takeaways:
  ...
  - Understand how Budgero converts multi-currency accounts using daily exchange rates and manual fallbacks.
  - Change the budget currency confidently and learn how the recalculation behaves.
---
...
- Budgero pulls official exchange rates daily and caches them in the budget file. How long the cache is kept is configurable in Settings → Currencies (30 days by default) — older rates are pruned to keep the budget file small and refetched on demand.
- When a fresh rate is needed, Budgero fetches it from the exchange-rate service and stores it. When an online session starts, it also refreshes the official rates for the current day so cached rates do not silently remain stale. If you're offline and no cached rate exists, you can supply a manual rate so the conversion can proceed; that manual rate is replaced automatically once you reconnect, unless you turn the "update offline rates" option off — and rates you pin on a transaction are never touched.
...
- When you reconnect, Budgero replaces offline conversions with official rates automatically — an opt-in that is on by default and can be changed in Settings → Currencies or right in the offline rate prompt.
- If the exact day is not cached, Budgero uses the nearest earlier rate up to a week back. Beyond that it asks you for a rate, prefilled with the closest cached one.

## Troubleshooting

- **Converted totals look off:** Check that a rate exists for the transaction date. Reopen the account or refresh the dashboard to prompt Budgero to fetch or recalculate the rate.
