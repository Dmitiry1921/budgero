...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>Funds underfunded <a href="/docs/goals">goals</a> by category priority, then shares money within a priority tier</td>
    </tr>
    <tr>
...
**Fund Goals** follows category priorities, independent of category or group order. Set priorities in the category row menu, Edit Category, or the side panel for selected categories. **Settings → Budgeting → Goal funding** controls the priority choices, proportional-shortfall or equal-completion sharing, and priority badges. See [Category funding priorities](/docs/goals#category-funding-priorities) for examples.

The side panel funds only selected categories, or the whole budget when none are selected. Unless **Allow over-assignment** is enabled, automatic goal funding stops when positive Ready to Assign runs out. All assignments from one funding action can be undone together.
